...
public class ToolsMenu extends GameObject {
	...
	public ToolsMenu(int x, int y, int width, int height, Grid grid, ObjectId id) {
	...
	public void tick() {
		if (MouseInput.leftPressed) {
			...
			if (getBoundsClear().contains(MouseInput.x, MouseInput.y))
				grid.clear();
		}
	}
	...
	public void render(Graphics g) {
		...
		for (int i = 0; i < buttons.size(); i++) {
			...
			if (getBoundsClear().contains(MouseInput.x, MouseInput.y)) {
				g2d.setColor(Color.white);
				g2d.fill(getBoundsClear());
				g2d.setColor(Color.black);
				g2d.draw(getBoundsClear());
				g.setFont(new Font("", Font.PLAIN, 18));
				g.drawString("Clear", (int) getBoundsClear().getX() + 14, (int) getBoundsClear().getY() + 22);
			}
			else {
				g2d.setColor(Color.black);
				g2d.fill(getBoundsClear());
	...
	public Rectangle getBoundsClear() {
		int width = GameMain.SCREEN_WIDTH / 10;
		int height = 30;
		return new Rectangle(GameMain.SCREEN_WIDTH - width - 10, (50 - height) / 2, width, height);
	}
